...
class SearchersAPI(object):
    ...
    @bases.with_orm_session
    def get_name(self, searcher_id, session=None):
        result = session.query(bases.Searchers.name).filter_by(\
                searcher_id=searcher_id).first()
        return result
    
    
    @bases.with_orm_session
    def get_tag(self, searcher_id, session=None):
        result = session.query(bases.Searchers.tag).filter_by(\
                searcher_id=searcher_id).first()
        return result
